...
//---------------------------------------------------------------------------------------------------------
	   ...
	   1) 서버와 클라에서 TCP가 유니크하게 식별되는 과정!!
		...
		- 여기는 커넥션이 없어 속도가 빠르지만, 추적 불가하므로 분실에 대한 책임 없음.
		- <src ip, src port, dest ip, dest port> 정보로 상대구분
		
		- TCP는 분실이 일어나면 책임지고 다시 전송한다
		- 혼잡한 상황이면 전체가 아닌 일부를 보내 제어한다
		- 고려할것이 많아 속도가 UDP에 비해 느리다.
		- TCP는 데이터를 보내는 경계가 없음, 데이터가 쪼개지거나 뭉쳐져서 보내질수있음.
