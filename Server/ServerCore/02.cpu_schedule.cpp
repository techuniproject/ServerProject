...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	[멀티 코어 CPU와 병렬 처리]
	- 멀티 코어 시스템에서 CPU 스케줄링과 작업 분배

	[1] 컨텍스트 스위칭 후 오버헤드 - 코어 친화성(캐시 친화성)으로 해결
	 -> 각 코어는 독립적인 캐시 메모리를 가지고 있다.
	 - 만일 CPU 스케줄러에 의해 어떤 코어에 실행시키기로 결정된 스레드가 이전에 해당 코어에서
	    실행된 적이 없다면, 그 스레드가 실행된 이후 스레드의 코드와 데이터가 캐시에 적재되는 데 
		  시간이 소요된다.
	 - 만일 CPU 스케줄러가 코어에서 최근에 실행된 적이 있는 스레드 중 하나를 선택한다면 캐시를 
	    다시 채우는 과정이 줄어들 수 있다. (L1,L2등 데이터가 남아있다면 캐시히트율이 높음)

	 - 해당 문제는 스레드를 동일한 코어에서만 계속 실행되도록 스케줄하는 CPU 친화성(CPU affinity)로 해결.
	 -> CPU 친화성은 프로세스나 스레드가 특정 CPU에서만 실행되도록 제한하는 스케줄링 특성.
	 -> 캐시와의 관련성 때문에 캐시 친화성이라고도 함. 멀티 코어 시스템에서는 코어 친화성으로 통함.

   [2] 코어별 부하 불균형 - 부하 균등화 기법으로 해결
    -> 멀티 코어 시스템에서 운영체제가 스레드를 무작위로 스케줄링 한다면 어떤 코어는 많은 수의
	    스레드를 어떤 코어는 작은 수의 스레드를 실행한다.
	-> 이렇게 코어 사이의 부하 불균형이 초래되면, 많은 스레드가 몰린 코어에서 실행되는 대기
	    시간과 처리 시간이 길어지는 한편, 어떤 코어는 실행할 스레드가 없어 노는 상태가 되어
		 코어의 활용률과 함께 시스템 처리율이 떨어진다.
	[부하 균등화 기법 (Load Balancing)]
	 - 운영체제는 코어 사이의 부하 불균형을 해결하기 위해 푸시, 풀 마이그레이션 기법을 사용.
	  
	  1) 푸시 마이그레이션
	   - 시스템에 스레드 큐를 감시하는 별도의 감시 스레드를 두고, 스레드 큐가 매우 짧거나 실행할
	      스레드가 없는 코어가 생길 때, 감시 스레드는 다른 스레드 큐로부터 스레드를 강제로 옮겨 놓는 기법.
	  
	  2) 풀 마이그레이션 
	   - 처리할 스레드가 없게 될 때 다른 코어의 스레드 큐에서 스레드를 가져와 자신의 스레드 큐에 넣고 실행하는 기법.
