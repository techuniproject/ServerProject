...
/*
HANDLE이나 SOCKET은 OS 커널이 리소스를 추상적으로 관리하기 위한 정수 핸들 값
포인터처럼 생겼지만 void*(HANDLE), uintptr_t(SOCKET) 실제 포인터 아님.

커널 내부에서는 이를 인덱스나 포인터 처럼 사용하고, 외부에서는 읽거나 쓰지 말고 오직 OS API를
통해서 사용해야 함.
