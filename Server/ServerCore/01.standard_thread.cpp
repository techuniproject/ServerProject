...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		[ 멀티스레드 구현 ] - 스레드 라이브러리와 커널 작동 및 협력
      
	  -> 응용프로그램에서 생성된 사용자 레벨 스레드의 코드가 CPU에 의해 실행되려면, 
		  궁극적으로 커널에 의해 스케줄되어야 한다. 
	  -> 커널은 커널 레벨 스레드밖에 모르기 때문에, 필연적으로 사용자 레벨 스레드와
	      커널 레벨 스레드가 매핑되어야 한다. (But std::thread는 커널 레벨로 1:1로 매핑이라고 생각)

		[1 : 1 매핑]
	  -> 1:1 매핑은 가장 단순한 매핑 방법으로, 사용자 레벨 스레드 하나당 커널에 의해 스케줄 가능한
	      엔터티 즉 TCB 하나를 연계시킨다. 커널이 스케줄링을 통해 TCB를 선택하면, CPU에게
		   이 TCB에 연결된 사용자 레벨 스레드의 코드를 실행하도록 하는 기법이다.
	  -> 커널이 응용프로그램을 적재하면, 커널 공간에 메인스레드 TCB_main을 생성하고 TCB_main의 시작주소(PC)에
	      main()함수의 주소를 기록한다.
	  -> 이후, 커널은 TCB_main를 스케줄하여 남는 코어에 할당하고, 코어는 TCB_main에 기록된 주소에서
		  실행을 시작한다. 
	  -> main()함수에서 스레드 라이브러리의 함수를 호출하여 새 스레드를 생성하면, 이 함수는 
	      새로운 사용자 레벨 스레드를 생성할 뿐 아니라 시스템 호출을 통해 커널 레벨 스레드를 생성해 줄 것으로 요청한다.
	  -> 이 결과 새로운 TCB가 생기고 해당 TCB 시작 주소에 사용자 스레드 코드의 주소가 기록된다.
	  -> 이런 식으로 응용프로그램에서 사용자 레벨 스레드를 생성하면 서로 다른 코어에서 동시에 실행되게 된다.
	  -> 1:1 매핑이 구현되기 위해서는 운영체제가 커널 레벨 스레드를 만드는 시스템 호출을 제공해야하고,
	      스레드 라이브러리는 사용자가 스레드를 만들 때마다 시스템 호출을 이용하여 커널 레벨 스레드를 생성해야한다.
			
		현대 std::thread -> 사용자 코드에서 만든 커널 스레드 핸들 -> 커널이 관리하고 스케줄링하는 객체

-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		
		[멀티 스레딩으로 응용프로그램을 작성하는 장점]
	 
	 1) 높은 실행 성능
	  - 입출력 작업과 CPU의 사용 시간이 많은 계산 중심 작업을 각각 스레드로 만들면 스레드들이 병렬적으로 처리되어
	     CPU 활용률이 높아진다. 
	  - 멀티코어 CPU를 가진 컴퓨터에서 스레드들이 여러 코어에서 병렬 실행되어 응용프로그램의 실행 시간이 단축된다.

	 2) 사용자에 대한 우수한 응답성
	  - 응용프로그램을 멀티스레드로 작성하면 한 스레드가 블록되어도, 다른 스레드들은 여전히 스케줄링되어 실행되기
	     때문에, 응용프로그램 전체가 블록되는 일이 일어나지 않는다. 
		  
	 3) 서버 프로그램의 우수한 응답성
	  - 웹 서버나 파일 서버 등 서버 프로그램들은 많은 사용자들의 동시 접근에 대한 사용자 응답성이 높아야 한다.
	  - 멀티스레딩은 접근하는 각 사용자마다 스레드를 할당하여 서비스함으로써, 동시에 접근하는 많은 사용자들에게 높은 응답성을 준다.
	  -> 요즘은 스레드 풀로 스레드 미리 개수 정해서 만들어놓고, 사용자가 작업을 요청하면 할당하는 방식으로 처리.
	 
	 4) 시스템 자원 사용의 효율성
	  - 스레드는 프로세스에 비해 생성하고 유지하는데 메모리나 자원이 적게 사용된다.

	 5) 응용프로그램 구조의 단순화
	  - 멀티스레드를 사용하면 응용프로그램의 구조를 단순화할 수 있다. 일반적으로 응용프로그램은 독립적인 여러 작업
	   들로 분할 가능하다. 각 작업은 함수 수준의 간단한 루틴으로 작성 가능한데, 이 함수들을 각각 스레드로 만든다면
	    쉽게 멀티스레드 프로그램을 설계하고 코딩할 수 있다.
	  - 응용프로그램에 새로운 작업을 덧붙이는 경우 새로운 스레드를 만들어 그 작업을 하게 할 수 있기 때문에 확장이나 변환이 용이하다.

	 6) 작성하기 쉽고 효율적인 통신
	  - 스레드 사이의 통신은 프로세스 사이의 통신에 비해 매우 간단하며 코딩하기 편리하다
	  - 그것은 스레드들이 프로세스의 주소 공간을 공유하기 때문이다.
	  - 스레드 사이에 데이터를 주고받을 때 프로세스의 전역변수나 동적 할당 메모리를 사용하면, 주고받는 속도 역시 매우 빠름.
